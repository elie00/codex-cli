...
import { normalizeMarkdownCodeBlocks } from "../../utils/code-language-normalizer";
  ...
  return (
    <Box gap={1} flexDirection="column">
      <Box gap={1}>
        ...
        <Text dimColor>pendant {thinkingTime}s</Text>
      </Box>
...
function TerminalChatResponseMessage({
  ...
  // Détecter le fournisseur à partir des métadonnées (si disponible)
  const providerName = 
    // @ts-expect-error les métadonnées peuvent ne pas exister dans le type
    message.metadata?.provider_name || 
    // @ts-expect-error accès au provider_name directement
...
function TerminalChatResponseToolCallOutput({
  ...
  if (message.type === "function_call_output" && !fullStdout) {
    ...
    if (lines.length > 4) {
      ...
      displayedContent = [...head, `... (${remaining} lignes de plus)`].join("\n");
    }
  }
...
export function Markdown({
  ...
  const rendered = React.useMemo(() => {
    try {
      // Normaliser les blocs de code dans le markdown
      const normalizedMarkdown = normalizeMarkdownCodeBlocks(children);
      
      // Créer d'abord l'instance du renderer avec toutes les options nécessaires
      const renderer = new TerminalRenderer({
        ...options,
        width: size.columns,
        // Assurez-vous que les fonctions de rendu sont bien définies
        code: (code, language) => {
        ...
        // Options supplémentaires pour le formatage
        colors: {
          ...(options.colors || {}),
          code: "cyan"
        }
      ...
      // Configurer marked avec le renderer
      setOptions({
        renderer: renderer
      });
      
      // Analyser le markdown
      const parsed = parse(normalizedMarkdown, { async: false }).trim();
